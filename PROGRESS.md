...
**Completion: 46.56%** — computed by `python scripts/completion.py`, never written by hand.
...
| `CDSW` vs ISDA test cases | 0.80 | **externally validated** — 0.33bp median against ISDA's own grid; residual is measured discretisation |
...
| Canvas with FDC3 | 0.90 | no *client* draws it — the server serves a workspace, both renderers still draw a single buffer |
